...
Provides both a Command-Line Interface (CLI) and an interactive prompt.
"""
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--duplicates",
        choices=["rename", "overwrite", "skip", "raise"],
        default="rename",
        help="Strategy for resolving duplicate filenames in destination.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
...
def interactive_mode() -> tuple[str, str | None, str, str, bool, str]:
    ...
    while True:
        ...
        if not source_path.exists():
            print(f"Directory not found: '{source_input}'. Please try again.")
            continue
        if not source_path.is_dir():
            print(f"Path is not a directory: '{source_input}'. Please try again.")
            continue
        break
    ...
    print("\nHow should unsupported file extensions be handled?")
    print("  1. Skip them (leave in source directory) [Default]")
    print("  2. Move to an 'Others/' folder")
    print("  3. Raise an error and stop")
    ...
    print("\nHow should duplicate file names be handled?")
    print("  1. Automatically rename (e.g. photo (1).jpg) [Default]")
    print("  2. Overwrite existing file")
    print("  3. Skip duplicate file")
    print("  4. Raise an error and stop")
    dup_choice = input("Select option (1/2/3/4): ").strip()
    dup_map = {"1": "rename", "2": "overwrite", "3": "skip", "4": "raise"}
    duplicate_strategy = dup_map.get(dup_choice, "rename")
...
def main() -> None:
    ...
    print(f" Duplicate Strategy     : {duplicate_strategy}", flush=True)
    ...
    try:
        ...
        print(f"\n[ERROR] File/Folder not found: {e}", file=sys.stderr)
        sys.exit(1)
    except UnsupportedFileError as e:
        print(f"\n[ERROR] Unsupported File Type: {e}", file=sys.stderr)
        sys.exit(1)
    except DestinationFolderError as e:
        print(f"\n[ERROR] Destination Folder Error: {e}", file=sys.stderr)
        sys.exit(1)
    except FileOrganizerError as e:
        print(f"\n[ERROR] Organizer Error: {e}", file=sys.stderr)
        sys.exit(1)
